...
function isMatch(s, p) {
    ...
    function runTest(string, pattern, callback) {
        var input = {
            s: string,
            p: pattern,
            iterations: 100
        ...
        var isRegexBased = $('#compare-opt-regex').is(':checked');
        var baseFnName = isRegexBased ? 'isMatchRegex' : 'isMatch';
        var baseFn = isRegexBased ? isMatchRegex : isMatch;
        ...
        app.compile(baseFnName, baseFn.toString()).invoke(input, function (err, baseRes) {
            ...
            if (code) {
                code.invoke(input, function (err, res) {
                    ...
                    var $inputString = $('#input-string', $clone);
                    var $inputPattern = $('#input-pattern', $clone);
                    var $expectedOutput = $('#expected-output', $clone);
                    var $actualOutput = $('#actual-output', $clone);
                    ...
                    $expectedOutput.text(baseRes.answer);
                    $actualOutput.text(res.answer);
                    if (baseRes.answer === res.answer) {
                        $actualOutput.addClass('text-success');
                        summary.passed++;
                    } else {
                        $actualOutput.addClass('text-danger');
                        summary.failed++;
                    }
    ...
    $('#run-once').on('click', function () {
        ...
        runTest($string.val(), $pattern.val(), function (err) {
            if (err) {
                console.log(err);
            }
            app.progress.hide();
